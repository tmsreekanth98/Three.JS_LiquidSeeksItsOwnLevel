...
//Liquid Maintains its own level
...
function initialiseVariables(){
  PIEscene.background = new THREE.Color(0x101010);
  pipe1status=0;
  pipe2status=0;
  pipe3status=0;
  pipe4status=0;
...
{
    helpContent="";
    helpContent = helpContent + "<h2>Liquid seeks its own level Experiment Help</h2>";
    helpContent = helpContent + "<h3>About the experiment</h3>";
    helpContent = helpContent + "<p>The experiment shows four different shaped vessels on a table connected at the bottom. The user can fill any of the vessels using the four taps provided. Upon filling any vessel the water level will uniformly rise in all the vessels.</p>";
    helpContent = helpContent + "<h3>Animation control</h3>";
    helpContent = helpContent + "<p>The animation is controlled by the four pipes/taps at the top. Clicking on a tap (white portion of pipe) causes it to go on and water will flow into the vessel. Switching on multiple taps at the same time is permissible.</p>";
    helpContent = helpContent + "<p>Once water starts filling in one or more of the vessels, the water level of all tubes rise. </p>";
    ...
    helpContent = helpContent + "<p>The water level will increase at the same level for all the vessels irrespective of the shape of the vessel.</p>";
...
{
    infoContent =  "";
    infoContent = infoContent + "<h2>Liquid seeks its own level Experiment Info</h2>";
    infoContent = infoContent + "<h3>About the experiment</h3>";
    infoContent = infoContent + "<p>The experiment shows four different shaped vessels on a table connected at the bottom. The user can fill any of the vessels using the four taps provided. Upon filling any vessel the water level will uniformly rise in all the vessels.</p>";
    infoContent = infoContent + "<h3>The Physics</h3><p>Water level rises uniformly due to water pressure. Water pressure depends only on height from surface and not on the shape of the vessel. Since any point in the same level of a connected vessel as such in the experiment will have same pressure, the water level rises uniformly.</p>";
    infoContent = infoContent + "<p>Irrespective of the type of the vessel the height /level of the liquid is the same. This happens because at the bottom,i.e at the long tube, the pressure should be the same.</p>";
...
function loadCenterTable(){
  ...
  texture.load( 'woodred.jpg', function(texture){
    ...
    //leg 1
      var geometry = new THREE.BoxGeometry(0.4,6,0.4);
      myTableTop = new THREE.Mesh(geometry, new THREE.MeshLambertMaterial({map:texture}));
      myTableTop.position.set(-3,-4.5,5.8)
      myTableTop.castShadow = true;
      PIEaddElement(myTableTop);
      //myTableTop.rotation.x+=Math.PI/16;
      myTableTop.castShadow=false;
    ...
    //leg 2
      ...
      var geometry = new THREE.BoxGeometry(0.4,6,0.4);
      myTableTop = new THREE.Mesh(geometry, new THREE.MeshLambertMaterial({map:texture}));
      myTableTop.position.set(-3.1,-4.2,1)
      myTableTop.castShadow = true;
      PIEaddElement(myTableTop);
      //myTableTop.rotation.x+=Math.PI/16;
      myTableTop.castShadow=false;
...
function loadExperimentElements(){
  ...
  PIEcreateTable("Observations", 5, 3, true);
  var header=["SNo","Level","Time"];
  ...
  PIEinputGUI.width=300;
  PIEaddInputSlider("Height(Connector 1)",0,connectorHeight0Handle,0,5,1);
  PIEaddInputSlider("Height(Connector 2)",0,connectorHeight1Handle,0,5,1);
  PIEaddInputSlider("Height(Connector 3)",0,connectorHeight2Handle,0,5,1);

  //Initial tube and block 4
  var geometry = new THREE.CylinderGeometry( 0.03,0.03,1,32 );
  var material = new THREE.MeshLambertMaterial({color:"white"});
  tube3 = new THREE.Mesh(geometry,material);
  PIEaddElement(tube3);
  tube3.position.set(1.3,-0.27,4.1);
  tube3.rotation.z-=Math.PI/2;
  tube3.rotation.x+=Math.PI/2;
  tube3.castShadow=false;

  var loader = new THREE.TextureLoader();
  loader.load('water.jpg',function(texture){
    var geometry = new THREE.CylinderGeometry( 0.03,0.03,1,32 );
    var material = new THREE.MeshLambertMaterial({map:texture});
    watertube3 = new THREE.Mesh(geometry,material);
    PIEaddElement(watertube3);
    watertube3.position.set(1.3,-0.27,4.1);
    watertube3.rotation.z-=Math.PI/2;
    watertube3.rotation.x+=Math.PI/2;
    watertube3.castShadow=false;
    watertube3.visible=false;
    watertube3.position.set(1.3,-0.27+c2Height/9,4.1);
    PIErender();
  });

  block2 = new THREE.Mesh(new THREE.BoxGeometry(1,0.7,1),new THREE.MeshLambertMaterial({color:"#404040"}));
  PIEaddElement(block2);
  block2.position.set(2,-0.65,4.01);
  var edges = new THREE.EdgesGeometry(new THREE.BoxGeometry(1,0.7,1));
  var line = new THREE.LineSegments( edges, new THREE.LineBasicMaterial( { color: 0x000 } ) );
  block2.add(line);

//Initial tube and block 3
  var geometry = new THREE.CylinderGeometry( 0.03,0.03,1.2,32 );
  var material = new THREE.MeshLambertMaterial({color:"white"});
  tube2 = new THREE.Mesh(geometry,material);
  PIEaddElement(tube2);
  tube2.position.set(0,-0.95,4.1);
  tube2.rotation.z-=Math.PI/2;
  tube2.rotation.x+=Math.PI/2;
  tube2.castShadow=false;

  var loader = new THREE.TextureLoader();
  loader.load('water.jpg',function(texture){
    var geometry = new THREE.CylinderGeometry( 0.03,0.03,1.2,32 );
    var material = new THREE.MeshLambertMaterial({map:texture});
    watertube2 = new THREE.Mesh(geometry,material);
    PIEaddElement(watertube2);
    watertube2.position.set(0,-0.95+c1Height/9,4.1);
    watertube2.rotation.z-=Math.PI/2;
    watertube2.rotation.x+=Math.PI/2;
    watertube2.castShadow=false;
    watertube2.visible=false;
  });

  block1 = new THREE.Mesh(new THREE.BoxGeometry(1,0.42,0.5),new THREE.MeshLambertMaterial({color:"#404040"}));
  PIEaddElement(block1);
  block1.position.set(0.7,-1.22,4.01);
  var edges = new THREE.EdgesGeometry(new THREE.BoxGeometry(1,0.42,0.5));
  var line = new THREE.LineSegments( edges, new THREE.LineBasicMaterial( { color: 0x000 } ) );
  block1.add(line);


  //1st beaker connector

  var loader = new THREE.TextureLoader();
  loader.load('water.jpg',function(texture){
    var geometry = new THREE.CylinderGeometry( 0.03,0.03,1.2,32 );
    var material = new THREE.MeshLambertMaterial({map:texture});
    watertube1 = new THREE.Mesh(geometry,material);
    PIEaddElement(watertube1);
    watertube1.position.set(-1.3,-0.95+c0Height/9,4.1);
    watertube1.rotation.z-=Math.PI/2;
    watertube1.rotation.x+=Math.PI/2;
    watertube1.castShadow=false;
    watertube1.visible=false;
    PIErender();
  });

  var geometry = new THREE.CylinderGeometry( 0.03,0.03,1.5,32 );
  var material = new THREE.MeshLambertMaterial({color:"white"});
  tube1 = new THREE.Mesh(geometry,material);
  PIEaddElement(tube1);
  tube1.position.set(-1.5,-0.95,4.1);
  tube1.rotation.z-=Math.PI/2;
  tube1.rotation.x+=Math.PI/2;
  tube1.castShadow=false;

  block0 = new THREE.Mesh(new THREE.BoxGeometry(1,0.42,0.5),new THREE.MeshLambertMaterial({color:"#404040"}));
  PIEaddElement(block0);
  block0.position.set(-2,-1.22,4.01);
  var edges = new THREE.EdgesGeometry(new THREE.BoxGeometry(1,0.42,0.5));
  var line = new THREE.LineSegments( edges, new THREE.LineBasicMaterial( { color: 0x000 } ) );
  block0.add(line);
...
//INPUT SLIDER HANDLING AND ANIMATIONS
var c0Height=0,c1Height=0,c2Height=1;

function connectorHeight0Handle(newval){
  c0Height=newval;
  if(flag==1){
    resetExperiment();
  }
  ...
  loader.load('water.jpg',function(texture){
    ...
    watertube1 = new THREE.Mesh(geometry,material);
    PIEaddElement(watertube1);
    watertube1.position.set(-2,-1+c0Height/9,4.01);
    watertube1.rotation.z-=Math.PI/2;
    watertube1.rotation.x+=Math.PI/2;
    watertube1.castShadow=false;
    watertube1.visible=false;
  ...
  tube1 = new THREE.Mesh(geometry,material);
  PIEaddElement(tube1);
  tube1.position.set(-1.3,-1+c0Height/9,4.1);
  tube1.rotation.z-=Math.PI/2;
  tube1.rotation.x+=Math.PI/2;
  tube1.castShadow=false;

  block0.visible=false;
  block0 = new THREE.Mesh(new THREE.BoxGeometry(1,0.42+c0Height/9,1),new THREE.MeshLambertMaterial({color:"#404040"}));
  PIEaddElement(block0);
  block0.position.set(-2,-1.22+c0Height/18,4.01);
  var edges = new THREE.EdgesGeometry(new THREE.BoxGeometry(1,0.42+c0Height/9,1));
  var line = new THREE.LineSegments( edges, new THREE.LineBasicMaterial( { color: 0x000 } ) );
  block0.add(line);

  beaker1.position.y=tube1.position.y+0.72;
  watertube.position.y=tube1.position.y;
  PIErender();
}

function connectorHeight1Handle(newval){
  ...
  tube2 = new THREE.Mesh(geometry,material);
  PIEaddElement(tube2);
  tube2.position.set(0,-0.95+c1Height/9,4.1);
  tube2.rotation.z-=Math.PI/2;
  tube2.rotation.x+=Math.PI/2;
  tube2.castShadow=false;

  block1.visible=false;
  block1 = new THREE.Mesh(new THREE.BoxGeometry(1,0.42+c1Height/9,0.5),new THREE.MeshLambertMaterial({color:"#404040"}));
  PIEaddElement(block1);
  block1.position.set(0.7,-1.22+c1Height/18,4.01);
  var edges = new THREE.EdgesGeometry(new THREE.BoxGeometry(1,0.42+c1Height/9,0.5));
  var line = new THREE.LineSegments( edges, new THREE.LineBasicMaterial( { color: 0x000 } ) );
  block1.add(line);

  beaker3.position.y=tube2.position.y+0.72;
...
function connectorHeight2Handle(newval){
  c2Height=newval;
  ...
  tube3 = new THREE.Mesh(geometry,material);
  PIEaddElement(tube3);
  tube3.position.set(1.3,-0.27+c2Height/9,4.1);
  tube3.rotation.z-=Math.PI/2;
  tube3.rotation.x+=Math.PI/2;
  tube3.castShadow=false;

  beaker4.position.y=tube3.position.y+0.38;
  //block2.scale.y=(c2Height+1)
  //block2.position.set(2,-0.65,4.01);
  block2.visible=false;
  block2 = new THREE.Mesh(new THREE.BoxGeometry(1,0.7+c2Height/9,1),new THREE.MeshLambertMaterial({color:"#404040"}));
  PIEaddElement(block2);
  block2.position.set(2,-0.65+(c2Height/18),4.01);
  var edges = new THREE.EdgesGeometry(new THREE.BoxGeometry(1,0.7+c2Height/9,1));
  var line = new THREE.LineSegments( edges, new THREE.LineBasicMaterial( { color: 0x000 } ) );
  block2.add(line);
...
//Function to load center Pipe
function loadPipes(){
  var loader = new THREE.TextureLoader();
  loader.load('pipe.jpg',function(texture){
    var horizontalPipe1 = new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,2.5,32),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(horizontalPipe1);
    PIEsetClick(horizontalPipe1,switch1Click);
    horizontalPipe1.position.set(-2,3,4);
    horizontalPipe1.castShadow=false;

    var horizontalPipe2 = new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,2.5,32),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(horizontalPipe2);
    PIEsetClick(horizontalPipe2,switch2Click);
    horizontalPipe2.position.set(-0.7,3,4);
    horizontalPipe2.castShadow=false;

    var horizontalPipe3 = new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,2.5,32),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(horizontalPipe3);
    PIEsetClick(horizontalPipe3,switch3Click);
    horizontalPipe3.position.set(0.7,3,4);
    horizontalPipe3.castShadow=false;

    var horizontalPipe4 = new THREE.Mesh(new THREE.CylinderGeometry(0.2,0.2,2.5,32),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(horizontalPipe4);
    PIEsetClick(horizontalPipe4,switch4Click);
    horizontalPipe4.position.set(2,3,4);
    horizontalPipe4.castShadow=false;
...
//Fucntion to load pipe switches
function loadSwitches(){
  var loader = new THREE.TextureLoader();
  loader.load('ring.jpg',function(texture){

    /**************1ST PIPE SWITCH*********************/
    switch1 = new THREE.Mesh(new THREE.TorusGeometry(0.3,0.05,16,100),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(switch1);
    switch1.castShadow=false;
    //PIEsetClick(switch1,switch1Click);

    var bar1 = new THREE.Mesh(new THREE.BoxGeometry(0.6,0.1,0.05),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(bar1);
    bar1.castShadow=false;
    //PIEsetClick(bar1,switch1Click);

    var bar2 = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.6,0.05),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(bar2);
    bar2.castShadow=false;
    //PIEsetClick(bar2,switch1Click);

    var circle = new THREE.Mesh(new THREE.CylinderGeometry(0.15,0.15,0.1,32),new THREE.MeshLambertMaterial({map:texture}));
    circle.rotation.x+=Math.PI/2;
    PIEaddElement(circle);
    circle.castShadow=false;
    //PIEsetClick(circle,switch1Click);


    SWITCH1 = new THREE.Group();
    SWITCH1.add(switch1);
    SWITCH1.add(bar1);
    SWITCH1.add(bar2);
    SWITCH1.add(circle);
    PIEaddElement(SWITCH1);
    PIEsetClick(SWITCH1,switch1Click);
    SWITCH1.position.set(-2,2.2,4.2);
    PIErender();


    /***************2ND PIPE SWITCH**************************/
    switch2 = new THREE.Mesh(new THREE.TorusGeometry(0.3,0.05,16,100),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(switch2);
    switch2.castShadow=false;
    //PIEsetClick(switch2,switch2Click);

    var bar1 = new THREE.Mesh(new THREE.BoxGeometry(0.6,0.1,0.05),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(bar1);
    bar1.castShadow=false;
    //PIEsetClick(bar1,switch2Click);

    var bar2 = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.6,0.05),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(bar2);
    bar2.castShadow=false;
    //PIEsetClick(bar2,switch2Click);

    var circle = new THREE.Mesh(new THREE.CylinderGeometry(0.15,0.15,0.1,32),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(circle);
    circle.castShadow=false;
    circle.rotation.x+=Math.PI/2;
    //PIEsetClick(circle,switch2Click);

    SWITCH2 = new THREE.Group();
    SWITCH2.add(switch2);
    SWITCH2.add(bar1);
    SWITCH2.add(bar2);
    SWITCH2.add(circle);
    PIEaddElement(SWITCH2);
    PIEsetClick(SWITCH2);
    SWITCH2.position.set(-0.7,2.2,4.2);


    /****************3RD PIPE SWITCH******************************/
    switch3 = new THREE.Mesh(new THREE.TorusGeometry(0.3,0.05,16,100),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(switch3);
    switch3.castShadow=false;
    //PIEsetClick(switch3,switch3Click);

    var bar1 = new THREE.Mesh(new THREE.BoxGeometry(0.6,0.1,0.05),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(bar1);
    bar1.castShadow=false;
    //PIEsetClick(bar1,switch3Click);

    var bar2 = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.6,0.05),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(bar2);
    bar2.castShadow=false;
    //PIEsetClick(bar2,switch3Click);

    var circle = new THREE.Mesh(new THREE.CylinderGeometry(0.15,0.15,0.1,32),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(circle);
    circle.castShadow=false;
    circle.rotation.x+=Math.PI/2;
    //PIEsetClick(circle,switch3Click);

    SWITCH3 = new THREE.Group();
    SWITCH3.add(switch3);
    SWITCH3.add(bar1);
    SWITCH3.add(bar2);
    SWITCH3.add(circle);
    PIEaddElement(SWITCH3);
    PIEsetClick(SWITCH3);
    SWITCH3.position.set(0.7,2.2,4.2);


    /****************4TH PIPE SWITCH*****************************/
    switch4 = new THREE.Mesh(new THREE.TorusGeometry(0.3,0.05,16,100),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(switch4);
    switch4.castShadow=false;
    //PIEsetClick(switch4,switch4Click);

    var bar1 = new THREE.Mesh(new THREE.BoxGeometry(0.6,0.1,0.05),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(bar1);
    bar1.castShadow=false;
    //PIEsetClick(bar1,switch4Click);

    var bar2 = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.6,0.05),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(bar2);
    bar2.castShadow=false;
    //PIEsetClick(bar2,switch4Click);

    var circle = new THREE.Mesh(new THREE.CylinderGeometry(0.15,0.15,0.1,32),new THREE.MeshLambertMaterial({map:texture}));
    PIEaddElement(circle);
    circle.castShadow=false;
    circle.rotation.x+=Math.PI/2;
    //PIEsetClick(circle,switch4Click);

    SWITCH4 = new THREE.Group();
    SWITCH4.add(switch4);
    SWITCH4.add(bar1);
    SWITCH4.add(bar2);
    SWITCH4.add(circle);
    PIEaddElement(SWITCH4);
    PIEsetClick(SWITCH4);
    SWITCH4.position.set(2,2.2,4.2);
...
//Pipe 1 ON/OFF control
function switch1Click(){
  if(pipe1status==0){
    ...
    PIErender();
    pipe1status=0;
    if(pipe1status==0&&pipe2status==0&&pipe3status==0&&pipe4status==0){
      waterDrip1.visible=false;
      PIErender();
      PIEstopAnimation();
    }
  }
...
//Pipe 2 ON/OFF control
function switch2Click(){
  if(pipe2status==0){
...
//Pipe 3 ON/OFF control
function switch3Click(){
  if(pipe3status==0){
    PIEstartAnimation();
    pipe3status=1;
    SWITCH3.rotation.z+=Math.PI/4;
    PIErender();
  }
  else{
    SWITCH3.rotation.z-=Math.PI/4;
...
//Pipe 4 ON/OFF control
function switch4Click(){
  if(pipe4status==0){
    PIEstartAnimation();
    pipe4status=1;
    SWITCH4.rotation.z+=Math.PI/4;
    PIErender();
  }
  else{
    SWITCH4.rotation.z-=Math.PI/4;
...
//Function to load Beakers
function loadBeakers(){
    var loader = new THREE.TextureLoader();
    loader.load('beaker.jpg',function(texture){
      var geometry = new THREE.CylinderGeometry( 0.3,0.3,1.5,32 );
      var material = new THREE.MeshLambertMaterial({map:texture,transparent:true,opacity:1});
      beaker1 = new THREE.Mesh(geometry,material);
      PIEaddElement(beaker1);
      beaker1.position.set(-2,-0.25,4);
      //beaker1.opacity=0.5;
      beaker1.castShadow=false;
      PIErender();

      var geometry = new THREE.CylinderGeometry( 0.3,0.08,1.5,32 );
      var material = new THREE.MeshLambertMaterial({map:texture});
      beaker2 = new THREE.Mesh(geometry,material);
      PIEaddElement(beaker2);
      beaker2.position.set(-0.7,-0.25,4);
      beaker2.castShadow=false;
      PIErender();

      var geometry = new THREE.CylinderGeometry( 0.15,0.15,1.5,32 );
      var material = new THREE.MeshLambertMaterial({map:texture});
      beaker3 = new THREE.Mesh(geometry,material);
      PIEaddElement(beaker3);
      beaker3.position.set(0.7,-0.25,4);
      beaker3.castShadow=false;
      PIErender();

      var geometry = new THREE.CylinderGeometry( 0.25,0.25,0.85,32 );
      var material = new THREE.MeshLambertMaterial({map:texture});
      beaker4 = new THREE.Mesh(geometry,material);
      PIEaddElement(beaker4);
      beaker4.position.set(2,0.10,4);
      beaker4.castShadow=false;
      PIErender();

      var geometry = new THREE.CylinderGeometry( 0.12,0.12,0.5,32 );
      var material = new THREE.MeshLambertMaterial({map:texture});
      beaker4part2 = new THREE.Mesh(geometry,material);
      //PIEaddElement(beaker4part2);
      beaker4part2.position.set(2,0.25,4);
      beaker4part2.castShadow=false;
...
function loadBeakerWater(){
  var blackHide = new THREE.Mesh(new THREE.BoxGeometry(4.4,3,0),new THREE.MeshLambertMaterial({color:0x040404}));
  PIEaddElement(blackHide);
  blackHide.castShadow=false;
  blackHide.position.set(0,-3,5);

  var loader = new THREE.TextureLoader();
  loader.load('water.jpg',function(texture){
    var geometry = new THREE.CylinderGeometry( 0.31,0.31,1.5,32 );
    var material = new THREE.MeshLambertMaterial({map:texture});
    beakerwater1 = new THREE.Mesh(geometry,material);
    PIEaddElement(beakerwater1);
    beakerwater1.position.set(-2,-1.65,4.01);
    beakerwater1.visible=false;
    beakerwater1.castShadow=false;
    PIErender();


    var geometry = new THREE.CylinderGeometry( 0.08,0.02,1.5,32 );
    var material = new THREE.MeshLambertMaterial({map:texture});
    beakerwater2 = new THREE.Mesh(geometry,material);
    PIEaddElement(beakerwater2);
    beakerwater2.position.set(-0.69,-1.65,4.25);
    beakerwater2.castShadow=false;
    beakerwater2.visible=false;
    PIErender();

    var geometry = new THREE.CylinderGeometry( 0.15,0.15,1.5,32 );
    var material = new THREE.MeshLambertMaterial({map:texture});
    beakerwater3 = new THREE.Mesh(geometry,material);
    PIEaddElement(beakerwater3);
    beakerwater3.position.set(0.7,-1.65,4.01);
    beakerwater3.castShadow=false;
    beakerwater3.visible=false;
    PIErender();

    //beakerwater3.position.y=beakerwater1.position.y;


    var geometry = new THREE.CylinderGeometry( 0.25,0.25,0.85,32  );
    var material = new THREE.MeshLambertMaterial({map:texture});
    beakerwater4= new THREE.Mesh(geometry,material);
    PIEaddElement(beakerwater4);
    beakerwater4.position.set(2,-1.33,4.02);
    beakerwater4.visible=false;
    beakerwater4.castShadow=false;
    PIErender();

    var geometry = new THREE.CylinderGeometry( 0.12,0.12,0.5,32 );
    var material = new THREE.MeshLambertMaterial({map:texture});
    beakerwater4part2 = new THREE.Mesh(geometry,material);
    //PIEaddElement(beakerwater4part2);
    beakerwater4part2.position.set(2,-0.25,4);
    beakerwater4part2.castShadow=false;
    PIErender();

    var geometry = new THREE.CylinderGeometry( 0.03,0.03,1.5,32 );
    var material = new THREE.MeshLambertMaterial({map:texture});
    watertube = new THREE.Mesh(geometry,material);
    PIEaddElement(watertube);
    watertube.position.set(-1.5,-0.95,4.1);
    watertube.rotation.z-=Math.PI/2;
    watertube.rotation.x+=Math.PI/2;
    watertube.castShadow=false;
...
//Function to load water.
function waterDrip(){
  /****Create Water*****/
  var loader=new THREE.TextureLoader();
  loader.load('dr.png',function(texture){
    waterDrip1 = new THREE.Mesh(new THREE.BoxGeometry(0.2,1.5,0),new THREE.MeshBasicMaterial({map:texture,transparent:true}));
    PIEaddElement(waterDrip1);
    waterDrip1.position.set(-2,1,4.05);
    PIEdragElement(waterDrip1);
    waterDrip1.visible=false;
    PIErender();

    waterDrip2 = new THREE.Mesh(new THREE.BoxGeometry(0.2,1.5,0),new THREE.MeshBasicMaterial({map:texture,transparent:true}));
    PIEaddElement(waterDrip2);
    waterDrip2.position.set(-0.7,1.9,4.01);
    waterDrip2.visible=false;
    PIErender();

    waterDrip3 = new THREE.Mesh(new THREE.BoxGeometry(0.2,1.5,0),new THREE.MeshBasicMaterial({map:texture,transparent:true}));
    PIEaddElement(waterDrip3);
    waterDrip3.position.set(0.7,1.9,4.01);
    waterDrip3.visible=false;
    PIErender();

    waterDrip4 = new THREE.Mesh(new THREE.BoxGeometry(0.2,1.5,0),new THREE.MeshBasicMaterial({map:texture,transparent:true}));
    PIEaddElement(waterDrip4);
    waterDrip4.position.set(2,1.9,4.01);
    waterDrip4.visible=false;
    PIErender();
  });
...
function MinHeight(a,b,c,d){
  var temp = (a < b)    ? a : b;
  var min =  (c < temp) ? c : temp;
  if(min<=d){
    return min;
  }
  else{
...
function updateExperimentElements(t,dt){
  if(t==0)
    return;
  //PIPE 1 water dripping
  if(pipe1status==1){
    waterDrip1.visible=true;
    //watertube.visible=true;
    //waterDrip1.position.y=1;
  }
  else{
    waterDrip1.visible=false;
  }

  //PIPE 2 water dripping.
  if(pipe2status==1){
    waterDrip2.visible=true;
    //watertube.visible=true;
    waterDrip2.position.y=1;
  }
  else{
    waterDrip2.visible=false;
  }

  //PIPE 3 water dripping.
  if(pipe3status==1){
    waterDrip3.visible=true;
    //watertube.visible=true;
    waterDrip3.position.y=1;
  }
  else{
    waterDrip3.visible=false;
  }

  //PIPE 4 water dripping.
  if(pipe4status==1){
    waterDrip4.visible=true;
    //watertube.visible=true;
    waterDrip4.position.y=1;
  }
  else{
    waterDrip4.visible=false;
  }

  if(pipe1status==1||pipe2status==1||pipe3status==1||pipe4status==1){
    flag=1;
    beakerwater1.visible=true;
    beakerwater2.visible=true;
    beakerwater3.visible=true;
    beakerwater4.visible=true;
    beakerwater4part2.visible=true;
    Wtext1.visible=true;
    Line.visible=true;
    //watertube.visible=true;
    //Filling 1st beaker
    var MinimumHeight = MinHeight(beaker1.position.y,beaker2.position.y,beaker3.position.y,beaker4.position.y);

    if(beakerwater1.position.y>=MinimumHeight){
      bfull=1;
      beakerwater1.position.y=beaker1.position.y;
      PIErender();
    }
    else{
      beakerwater1.position.y+=(0.1)*(dt/2500)*(pipe1status+pipe2status+pipe3status+pipe4status);
      Line.position.y+=(0.1)*(dt/2500)*(pipe1status+pipe2status+pipe3status+pipe4status);
      Wtext1.position.y+=(0.1)*(dt/2500)*(pipe1status+pipe2status+pipe3status+pipe4status);
      PIErender();
    }

    //Filling 2nd beaker.
    if(beakerwater2.position.y>=beaker2.position.y){
      beakerwater2.position.y=beaker2.position.y;
      PIErender();
    }
    else{
      beakerwater2.position.y+=(0.1)*(dt/2500)*(pipe1status+pipe2status+pipe3status+pipe4status);
      beakerwater2.scale.x+=1.9*(0.1)*(dt/2500)*(pipe1status+pipe2status+pipe3status+pipe4status);
    }

    //Filling 3rd beaker.
    if(beakerwater3.position.y>=beaker3.position.y){
      beakerwater3.position.y=beaker3.position.y;
      PIErender();
    }
    else{
      beakerwater3.position.y+=(0.1)*(dt/2500)*(pipe1status+pipe2status+pipe3status+pipe4status);
    }

    //Fillinf 4th beaker
    if(beakerwater4.position.y>=beaker4.position.y){
      beakerwater4.position.y=beaker4.position.y;
      PIErender();
      if(beakerwater4part2.position.y>=beaker4part2.position.y){
        beakerwater4part2.position.y=beaker4part2.position.y;
        PIErender();
      }
      else{
        beakerwater4part2.position.y+=(0.1)*(dt/2500)*(pipe1status+pipe2status+pipe3status+pipe4status);
      }
    }
    else{
      beakerwater4.position.y+=(0.1)*(dt/2500)*(pipe1status+pipe2status+pipe3status+pipe4status);
    }

    if(beakerwater1.position.y>=(tube3.position.y-0.73)){
      watertube3.position.y=tube3.position.y;
      watertube3.visible=true;
      //alert(beakerwater1.position.y);
      PIErender();
    }
    if(beakerwater1.position.y>=(tube2.position.y-0.73)){
      watertube2.position.y=tube2.position.y;
      watertube2.visible=true;

      PIErender();
    }

    if(beakerwater1.position.y>=tube1.position.y-0.73){
      watertube.visible=true;
    }

    if(bfull==1){
      alert("Vessels Filled Completely! Resetting scene.")
      waterDrip1.visible=false;
      waterDrip2.visible=false;
      waterDrip3.visible=false;
      waterDrip4.visible=false;
      ...
      if(trials<4){
        PIEupdateTableCell(trials+1,0,(trials+1));
        PIEupdateTableCell(trials+1,1,"Constant");
        PIEupdateTableCell(trials+1,2,t/1000);
        trials++;
      }
      else{
        alert('Table capacity Full! Reset to continue!');
      }
...
//Water Level Dashed Line Code
function loadWaterLevel(){
Line = new THREE.Mesh(new THREE.BoxGeometry(10,0.05,0),new THREE.MeshLambertMaterial({color:0xff9900}));
PIEaddElement(Line);
Line.position.set(0,-0.9,4);
Line.visible=false;
Line.castShadow=false;

var loader = new THREE.FontLoader();
loader.load('optimer.json',function(font){
  var geometry = new THREE.TextGeometry( 'WATER LEVEL', {
    font: font,
    size: 0.2,
    height: 0.05,
  });
  ...
  Wtext1.position.set(-5,-0.82,4);
  Wtext1.visible=false;
  Wtext1.rotation.y+=0.14;
  } );
...
//RESET EXPERIMENT FUNCTION

function resetExperiment(){
    flag=0;
  for(var i=1;i<=trials&&trials>=4;i++){
  ...
  loader.load('water.jpg',function(texture){
    ...
    PIEaddElement(beakerwater2);
    beakerwater2.position.set(-0.69,-1.65,4.25);
    //beakerwater2.position.set(-0.67,-1,4.5);
    beakerwater2.castShadow=false;
    beakerwater2.visible=false;
    PIErender();
  });
  ...
  beakerwater3.visible=false;
  beakerwater4.visible=false;
  beakerwater4part2.visible=false;
  watertube.visible=false;
  ...
  beakerwater2.position.set(-0.69,-1.65,4.25);
  beakerwater3.position.set(0.7,-1.65,4.01);
beakerwater4.position.set(2,-1.33,4);
  beakerwater4part2.position.set(2,-0.25,4);
  ...
  //block2.position.set(2,-0.65,4.01);
  tube2.position.set(0,-0.95,4.1);
  //block1.position.set(0.7,-0.77,4.01);
  tube1.position.set(-1.3,-0.95,4.1);
  tube3.position.set(1.3,-0.27,4.1);
